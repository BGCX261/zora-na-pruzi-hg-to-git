...
#            včíl zkusím ještě najít modul,  ak je v samostatném modulu
            ...
            if třída is None:
                raise AttributeError('Třída pro element {} nejestvuje v modulu {}'.format(klíč,  modul_třídy.__name__))

        self[klíč] = třída
        return třída


def davaj_parser(v_modulu,  výchozí_element = __ELEMENT):
    ...
    class Lookup(lxml.etree.CustomElementClassLookup):
        def lookup(self, node_type, document, namespace, name):
            if node_type == 'element':
                return třídy_elementů.get(name.upper(),  výchozí_element)
